...
contract Escrow is Ownable2Step {
    ...
    function fund(uint256 id, address token, uint256 amount, address payer) external payable onlyCore {
    ...
    function releaseWithFees(
        ...
        require(uint32(platformFeeBps) + uint32(protocolFeeBps) + uint32(callerTipBps) <= 10000, "total-bps");
